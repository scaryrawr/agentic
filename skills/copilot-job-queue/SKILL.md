...
allowed-tools: Bash(uv run scripts/job_queue.py:*)
compatibility: Requires uv. Git and the appropriate GitHub or Azure DevOps PR tooling are required when executing claimed coding jobs.
---
...
The helper emits JSON on stdout and errors as JSON on stderr. Run `uv run scripts/job_queue.py
